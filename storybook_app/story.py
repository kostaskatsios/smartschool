...
from .backgrounds import BackgroundTheme
...
class Storybook:
    ...
    def as_text(self) -> str:
        ...
        footer = [
            "-" * 60,
            f"Περιγραφή φόντου: {self.background.description}",
            "-" * 60,
        ]
        return "\n".join(header + body + footer)


def _build_page_texts(
    idea: Idea,
    hero: HeroOption,
    background: BackgroundTheme,
    style: IllustrationStyle,
    ...
    background_mood = {
        "Παιχνιδιάρικο Ουράνιο Τόξο": "ένα ουράνιο τόξο που αγκαλιάζει τον ορίζοντα",
        "Κομψό Μινιμαλιστικό Στούντιο": "ένα απαλό φως που λούζει τον χώρο σαν στούντιο",
    }.get(background.name, "ένα φωτεινό σκηνικό")

    pages = [
        (
            f"Η ιστορία ξεκινά όταν ο ήρωας {hero.title} κοιτά τον ουρανό και βλέπει "
            f"ένα σημάδι που μόνο εκείνος καταλαβαίνει. Γύρω του απλώνεται {background_mood} "
            f"ενώ το {base_tone} τον προσκαλεί να ακολουθήσει το μυστήριο της ιστορίας "
            f"'{idea.title}'."
        ),
        (
...
def create_storybook(
    ...
    pages = _build_page_texts(idea, hero, background, style)
    return Storybook(title=idea.title, background=background, style=style, pages=pages)
